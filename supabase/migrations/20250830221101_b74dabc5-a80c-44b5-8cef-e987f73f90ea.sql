...
-- Unique constraint: max 5 active keys per tenant
CREATE UNIQUE INDEX idx_api_keys_tenant_active_limit 
ON public.api_keys (tenant_id, active) 
WHERE active = true;
...
-- RLS policies
CREATE POLICY "idem_rw" ON public.idempotency_keys
FOR ALL USING (tenant_id = current_tenant_id())
WITH CHECK (tenant_id = current_tenant_id());

-- Unique constraint for idempotency
CREATE UNIQUE INDEX idx_idempotency_keys_unique 
ON public.idempotency_keys (tenant_id, idempotency_key, route);

-- TTL index for 24h cleanup
CREATE INDEX idx_idempotency_keys_ttl ON public.idempotency_keys (created_at);
